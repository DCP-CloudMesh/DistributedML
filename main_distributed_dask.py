...
def main():
    ...
    # Dask Clusters
    cluster = LocalCluster(processes=True, n_workers=num_partitions+1)
    with Client(cluster) as client:
        ...
        for epoch in range(epochs):
            ...
            # validation tasks
            criterion = nn.CrossEntropyLoss()
            tasks.append(dask.delayed(val)(base_model, device, val_loader, criterion, epoch, data_path))
            ...
            for i in range(num_partitions):
                queue = Queue()
                queues.append(queue)
                tasks.append(dask.delayed(train_model)(models[i], train_loaders[i], queue, epoch))
